...
def load_documents(data_dir):
    documents = []
    for file_path in glob.glob(os.paht.join(data_dir, "*.txt")):
...
def create_vector_store(data_dir):
    ...
    if not documents:
        logger.error("No documents found in seprate_data folder")
        return None
    ...
    if not qdrant_url or not qdrant_api_key:
        logger.error("Qdrant_URL and QDRANT_API_KEY key must be set in .env file")
        return None
    ...
    try:
        ...
        logger.info("Vector store created succcessfully")
        return vector_store
